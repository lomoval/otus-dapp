...
export function getSubscriptionPDA(
  ...
  return PublicKey.findProgramAddressSync(
    [Buffer.from('subscription'), owner.toBuffer(), serviceProvider.toBuffer()],
    programId
  );
}

export async function getSubscriptionData(program: Program<Counter>, subscriptionPDA: PublicKey) {
  try {
